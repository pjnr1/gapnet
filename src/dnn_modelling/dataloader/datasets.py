...
def get_gapposition_from_filename(x: str | Path):
    if isinstance(x, Path):
        x = x.name
...
def get_gap_closest_to_duration(file_list: List[str] | List[Path], duration_in_ms: float):
